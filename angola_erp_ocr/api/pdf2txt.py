...
def extract_text(
	...
	**kwargs: Any
	...
	with open(files, "rb") as fp:
		...
		#fff.append(fp.read())
		#fff.append(pdfminer.high_level.extract_text_to_fp(fp, **locals()))
	return outfp
...
def parse_args(args: Optional[List[str]]) -> argparse.Namespace:
	...
	print ('parsed_args ',parsed_args.files)

	parsed_args.files =  args
	print ('Dparsed_args ',parsed_args.files)
...
def extrair_texto(args):
	print ('args ', args)
	if os.path.isfile(frappe.get_site_path('public','files') + args.replace('/files','')):
		filefinal = frappe.get_site_path('public','files') + args.replace('/files','')
		...
		filefinal = args
	print ('Filefinal ', filefinal)
	parsed_args = parse_args(filefinal)
	outfp = extract_text_to_file(**vars(parsed_args))
	print ('+++++++++')
	print ('+++++++++')
	#print (type(outfp))
	#print (outfp.name)
