	...
	if ("MCX DEBIT" in ocr_tesserac or "MCX DÉBIT" in ocr_tesserac or "woY DEBIT" in ocr_tesserac or "iCx DEBIT" in ocr_tesserac or "Wotw DEBIT" in ocr_tesserac or "MCX DEBIE" in ocr_tesserac or "MOY DEBIT" in ocr_tesserac or "MCX PRET" in ocr_tesserac) and ("ZAP FIBRA" in ocr_tesserac or "ZP EIBRA " in ocr_tesserac or "ZAP  FIBRA" in ocr_tesserac):
		...
		for dd in ocr_tesserac.split('\n'):
			...
			if dd !='' and dd != ' ':
				...
				if "N.CAIXA:" in dd or "N-UAIXA:" in dd or "N.CATNA:" in dd or "W.CAIXA:" in dd or "N.CATXA:" in dd:
				...
				elif "MONTANTE:" in dd or "MONTANTE :" in dd or "MONTANTE;" in dd:
					#Pagamento feito
					if "MONTANTE :" in dd:
						dd0 = dd[10:].split(' ')
		...
		for dd in ocr_tesserac.split('\n'):
			...
			if dd !='' and dd != ' ':
				...
				elif "N.CAIXA:" in dd or "N-UAIXA:" in dd or "N.CATNA:" in dd or "W.CAIXA:" in dd or "N.CATXA:" in dd:
			...
			if "UN " in fsup.upper().strip() or "UNN/ " in fsup.upper().strip() or "/UN/\'" in fsup.upper().strip():
				...
				if "/UN/\'" in fsup.upper().strip():
					...
					if re.match(ending_currency_PT,tmp_fsup):
						...
						print ('NEW tmp_fsup ', tmp_fsup)
						print ('NEW FSUP ')
						print (fsup[:fsup.upper().find('UNN/ ')+5] + str(tmp_fsup))
						fsup = fsup[:fsup.upper().find('UNN/ ')+5] + str(tmp_fsup)

						#FIX 09-10-2023; DO again the search...
						tmp_fsup = fsup[fsup.upper().find('UNN/ ')+5:]
						patt_currency_dot = r'^([0-9]{1,3}\.[0-9]{2,3}\,[0-9]{2,3})'
						if re.match(patt_currency_dot,tmp_fsup):
							#Remove existing... assuming first is Price, second Discount and third is the prize
							pat = tmp_fsup[re.match(patt_currency_dot,tmp_fsup).span()[1]:].strip()
							#Check if 0.0%
